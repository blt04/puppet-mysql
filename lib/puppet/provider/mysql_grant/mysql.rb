...
Puppet::Type.type(:mysql_grant).provide(:mysql) do
	...
	def mysql_flush 
		mysqladmin mysql_opts, "flush-privileges"
	end
	...
	def create_row
		unless @resource.should(:privileges).empty?
			...
			case name[:type]
			when :user
				mysql mysql_opts, "mysql", "-e", "INSERT INTO user (host, user) VALUES ('%s', '%s')" % [
					name[:host], name[:user],
				]
			when :db
				mysql mysql_opts, "mysql", "-e", "INSERT INTO db (host, user, db) VALUES ('%s', '%s', '%s')" % [
	...
	def destroy
		mysql mysql_opts, "mysql", "-e", "REVOKE ALL ON '%s'.* FROM '%s@%s'" % [ @resource[:privileges], @resource[:database], @resource[:name], @resource[:host] ]
	end
	...
	def row_exists?
		...
		not mysql( mysql_opts, "mysql", "-NBe", 'SELECT "1" FROM %s WHERE %s' % [ name[:type], fields.map do |f| "%s = '%s'" % [f, name[f]] end.join(' AND ')]).empty?
	end
	...
	def privileges 
		...
		case name[:type]
		when :user
			privs = mysql mysql_opts, "mysql", "-Be", 'select * from user where user="%s" and host="%s"' % [ name[:user], name[:host] ]
		when :db
			privs = mysql mysql_opts, "mysql", "-Be", 'select * from db where user="%s" and host="%s" and db="%s"' % [ name[:user], name[:host], name[:db] ]
		end
	...
	def privileges=(privs) 
		...
		mysql mysql_opts, "mysql", "-Be", stmt
		mysql_flush
	end
end
